...
将耗时的AI计算移至异步线程，主线程只负责快照和写回，实现同tick内完成，实体行为完全一致。
...
实体Tick分批并行处理，充分利用多核CPU，大幅降低主线程负载。

**特性：**
- 实体级分批（8个实体一批，任务均衡）
- 自适应超时（根据MSPT动态调整）
- CompletableFuture聚合（消除长尾任务）

### 光照优化

Starlight算法16层分层队列，TPS自适应批量处理，光照更新去重。

### 性能优化套件

ServerCore启发的推挤、查找缓存、碰撞检测优化，覆盖主要性能热点。

### 内存优化套件

FerriteCore启发的对象池、谓词缓存、列表预分配，减少GC压力。
...
**Version**: 2.0.0 | **License**: GPL-3.0 | **Author**: [Virgil](https://github.com/virgil698)
