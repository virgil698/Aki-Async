...
public final class Hashing {
  ...
  private static final int BLAKE2B_ROUNDS = 12;

  private static final int MESSAGE_LENGTH = 16;

  private static final int CHAIN_VALUE_LENGTH = 8;

  private static final int INTERNAL_STATE_LENGTH = 16;

  private static final long PARAMETER_BLOCK = 0x01010040L;
  ...
  public static void hash(
    ...
    if (chainValue.length != CHAIN_VALUE_LENGTH) {
      throw new IllegalArgumentException("Chain value must be 8 longs");
    }
    if (internalState.length != INTERNAL_STATE_LENGTH) {
      throw new IllegalArgumentException("Internal state must be 16 longs");
    }
    ...
    for (int round = 0; round < BLAKE2B_ROUNDS; round++) {
      ...
      mixG(message[sigma[14]], message[sigma[15]], 3, 4, 9, 14, internalState);
    }

    for (int i = 0; i < CHAIN_VALUE_LENGTH; i++) {
      chainValue[i] ^= internalState[i] ^ internalState[i + CHAIN_VALUE_LENGTH];
    }
  }

  private static void mixG(
      long messageWord1,
      long messageWord2,
      int posA,
      int posB,
