...
public class Hashing {
    ...
    public static void hash(long[] message, long[] chainValue, long[] internalState, long messageOffset, boolean isFinal) {
        ...
        else internalState[14] = BLAKE2B_IV[6];
        internalState[15] = BLAKE2B_IV[7];

        for (int round = 0; round < 12; round++) {
            byte[] sigma = BLAKE2B_SIGMA[round];
            G(message[sigma[0]], message[sigma[1]], 0, 4, 8, 12, internalState);
            G(message[sigma[2]], message[sigma[3]], 1, 5, 9, 13, internalState);
            G(message[sigma[4]], message[sigma[5]], 2, 6, 10, 14, internalState);
            G(message[sigma[6]], message[sigma[7]], 3, 7, 11, 15, internalState);
            G(message[sigma[8]], message[sigma[9]], 0, 5, 10, 15, internalState);
            G(message[sigma[10]], message[sigma[11]], 1, 6, 11, 12, internalState);
            G(message[sigma[12]], message[sigma[13]], 2, 7, 8, 13, internalState);
            G(message[sigma[14]], message[sigma[15]], 3, 4, 9, 14, internalState);
        }
