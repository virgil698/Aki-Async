...
public final class WitchCpuCalculator {
     ...
     * Purified: Use Holder<MobEffect> object comparison (no String)
     * 
     * @param snapshot Witch snapshot
     * @return Potion type name, null if none needed
     */
    private static String decidePotionToDrink(WitchSnapshot snapshot) {
        // Simplified CPU simulation: Just make thread busy
        // No actual potion logic (too complex for async without registry access)
        double health = snapshot.getHealth();
        int effectCount = snapshot.getActiveEffects().size();
        
        // CPU-intensive computation (simulate complex logic)
        double score = 0.0;
        for (int i = 0; i < 100; i++) {
            score += Math.sqrt(health + effectCount + i);
        }
        
        // Decision based on computation
        if (health < 8.0 && score > 100.0) {
            return "healing";  // Simplified return
        }
