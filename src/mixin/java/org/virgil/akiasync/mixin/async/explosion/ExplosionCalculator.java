...
public class ExplosionCalculator {
    ...
    public ExplosionResult calculate() {
        ...
        if (bridge != null && bridge.isTNTUseUnifiedEngine()) {
            return calculateWithUnifiedEngine(bridge);
        }

        return calculateLegacy(bridge);
    }

    private ExplosionResult calculateWithUnifiedEngine(org.virgil.akiasync.mixin.bridge.Bridge bridge) {
        if (bridge != null && bridge.isTNTDebugEnabled()) {
            bridge.debugLog("[AkiAsync-TNT] Using UnifiedExplosionEngine");
        }

        UnifiedExplosionEngine engine = UnifiedExplosionEngine.getOrCreate(snapshot.getLevel());
        return engine.calculate(snapshot.getCenter(), snapshot.getPower(), snapshot.isFire());
    }

    private ExplosionResult calculateLegacy(org.virgil.akiasync.mixin.bridge.Bridge bridge) {
        if (bridge != null && bridge.isTNTDebugEnabled()) {
            bridge.debugLog("[AkiAsync-TNT] ExplosionCalculator.calculate() started (legacy mode)");
        }
                            ...
                            if (rayPower > 0.0f && !destroyedBlocks.containsKey(pos)) {
                                ...
                                if (snapshot.isProtected(pos)) {
                                    org.virgil.akiasync.mixin.bridge.Bridge bridge =
                                        org.virgil.akiasync.mixin.bridge.BridgeManager.getBridge();
                                    if (bridge != null && bridge.isTNTDebugEnabled()) {
                                        bridge.debugLog("[AkiAsync-TNT] Block at " + pos + " is protected, skipping");
                                    }
                                    continue;
                                }
    ...
    private double calculateExposure(Vec3 explosionCenter, ExplosionSnapshot.EntitySnapshot entity) {
        ...
        if (!useFullRaycast && bridge != null && bridge.isTNTUseSakuraDensityCache()) {
        ...
        double stepMultiplier = useFullRaycast ? 2.0 : 2.0;
        ...
        double offsetX = (1.0 - Math.floor(1.0 / stepX) * stepX) * 0.5;
        double offsetZ = (1.0 - Math.floor(1.0 / stepZ) * stepZ) * 0.5;
