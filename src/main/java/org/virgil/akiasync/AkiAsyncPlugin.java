...
public final class AkiAsyncPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("");
        getLogger().info("鈥?Core Features:");
        getLogger().info("  鈥?Async Entity Tracker: " + (configManager.isEntityTrackerEnabled() ? "Enabled" : "Disabled"));
        getLogger().info("  鈥?Async Mob Spawning: " + (configManager.isMobSpawningEnabled() ? "Enabled" : "Disabled"));
        getLogger().info("  鈥?Entity Tick Parallel: " + (configManager.isEntityTickParallel() ? "Enabled" : "Disabled") + " (" + configManager.getEntityTickThreads() + " threads)");
        getLogger().info("  鈥?Async Lighting: " + (configManager.isAsyncLightingEnabled() ? "Enabled" : "Disabled") + " (" + configManager.getLightingThreadPoolSize() + " threads)");
        getLogger().info("");
        getLogger().info("鈥?Performance Settings:");
        getLogger().info("  鈥?Thread Pool Size: " + configManager.getThreadPoolSize());
        getLogger().info("  鈥?Max Entities/Chunk: " + configManager.getMaxEntitiesPerChunk());
        getLogger().info("  鈥?Brain Throttle: " + (configManager.isBrainThrottleEnabled() ? "Enabled" : "Disabled") + " (" + configManager.getBrainThrottleInterval() + " ticks)");
        getLogger().info("  鈥?Update Interval: " + configManager.getUpdateIntervalTicks() + " ticks");
        getLogger().info("");
        getLogger().info("鈥?Optimizations:");
        getLogger().info("  鈥?ServerCore optimizations: Enabled");
        getLogger().info("  鈥?FerriteCore memory optimizations: Enabled");
        getLogger().info("  鈥?16-layer lighting queue: Enabled");
        getLogger().info("========================================");
